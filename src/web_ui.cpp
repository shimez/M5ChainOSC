...
function focusEncoderAfterMigration(uid){sessionStorage.setItem('m5osc-focus-encoder-migration',uid)}
function startEncoderV2Migration(index,uid){focusEncoderAfterMigration(uid);location.href='/?encoder_v2_migration='+encodeURIComponent(index)+'&encoder_uid='+encodeURIComponent(uid)}
function cancelEncoderV2Migration(uid){focusEncoderAfterMigration(uid);location.href='/'}
function confirmEncoderV2Migration(form){let migration=form.querySelector('input[name^="ev2m_"]');if(!migration)return true;let index=migration.name.substring(5),confirm=form.querySelector('input[name="ev2c_'+index+'"]:checked');if(confirm)return true;alert(tx('Confirm the semantic differences before saving the v2 candidate.','v2候補を保存する前に、動作上の違いを確認してください。'));return false}
...
function initializePage(){let form=document.getElementById('settings-form');if(form){form.addEventListener('input',markDirty);form.addEventListener('change',markDirty);if(form.querySelector('input[name^="ev2m_"]'))history.replaceState(null,'',location.pathname)}initializeMessageRows();document.querySelectorAll('.device[data-collapse-key]').forEach(card=>{let key=card.dataset.collapseKey,index=card.dataset.deviceIndex;if(sessionStorage.getItem('m5osc-collapse-'+key)==='1'){let body=document.getElementById('device-body-'+index),button=document.getElementById('collapse-'+index);body.hidden=true;button.classList.add('collapsed');button.setAttribute('aria-expanded','false')}});let focusUid=sessionStorage.getItem('m5osc-focus-encoder-migration');if(focusUid!==null){sessionStorage.removeItem('m5osc-focus-encoder-migration');let card=[...document.querySelectorAll('.device[data-collapse-key]')].find(x=>x.dataset.collapseKey===focusUid);if(card)requestAnimationFrame(()=>card.querySelector('.enc')?.scrollIntoView({behavior:'smooth',block:'start'}))}else{let saved=sessionStorage.getItem('m5osc-scroll');if(saved!==null){sessionStorage.removeItem('m5osc-scroll');requestAnimationFrame(()=>window.scrollTo(0,Number(saved)||0))}}document.querySelectorAll('form:not(#settings-form)').forEach(f=>f.addEventListener('submit',rememberScroll))}
window.settingsDirty=false;window.settingsSubmitting=false;window.addEventListener('pageshow',()=>window.settingsSubmitting=false);window.addEventListener('beforeunload',e=>{if(window.settingsDirty&&!window.settingsSubmitting){e.preventDefault();e.returnValue=''}});window.addEventListener('DOMContentLoaded',initializePage)
  ...
  for (int i = 0; i < deviceCount; i++) {
    ...
    if (devices[i].type == CHAIN_KEY_TYPE_CODE) {
      ...
      if (encoderV2)
        html += "<span class='encoder-model encoder-model-v2'>" + String(encoderMigrationEditing ? tr("V2 migration candidate", "V2移行候補") : tr("V2 settings", "V2設定")) + "</span>";
      if (!encoderV2) {
        html += "<div class='encoder-legacy-status'><strong>" + String(tr("Legacy Encoder settings", "旧形式のエンコーダー設定")) + "</strong><span>" + String(tr("Ordinary Save keeps the Legacy model.", "通常の保存では旧形式のまま維持されます。")) + "</span><button class='encoder-migration-action' type='button' onclick=\"startEncoderV2Migration(" + idx + ",'" + htmlEscape(devices[i].uid) + "')\">" + String(tr("Migrate to v2 settings", "v2設定へ移行する")) + "</button></div>";
      }
      if (encoderMigrationEditing) {
        html += "<div class='encoder-migration-panel'><strong>" + String(tr("Review the notes about migrating to v2", "v2形式への移行に関する注意事項")) + "</strong><ul>";
        if (devices[i].enc.sendIncrement) {
          ...
          if (devices[i].enc.absInMin != 0.0f)
            html += "<li>" + String(tr("The Legacy absolute-input offset is not represented in v2 Amount", "旧形式の絶対値入力オフセットはv2回転量では表現されません")) + "</li>";
          if (encoderUi.rangeSteps == 0)
            html += "<li>" + String(tr("Enter Range Steps from 1 to 65535 before saving.", "保存前に範囲ステップ数を1～65535で入力してください。")) + "</li>";
        }
        if (encoderUi.rotationMode == ENCODER_ROTATION_AMOUNT && !(encoderUi.outputMin < encoderUi.outputMax))
          html += "<li>" + String(tr("Set Minimum lower than Maximum before saving.", "保存前に最小値を最大値より小さく設定してください。")) + "</li>";
        html += "</ul><label class='encoder-migration-confirm'><input type='checkbox' name='ev2c_" + idx + "' value='1'>" + String(tr("I understand the differences and want to save as v2.", "動作上の違いを理解し、v2として保存します。")) + "</label><a class='encoder-migration-cancel' href='/' onclick=\"cancelEncoderV2Migration('" + htmlEscape(devices[i].uid) + "');return false\">" + String(tr("Return to Legacy settings", "旧形式の設定へ戻る")) + "</a></div>";
        html += "<input type='hidden' name='ev2m_" + idx + "' value='1'>";
      }
      html += "<div class='encoder-grid'>";
      html += addressInputHtml(tr("OSC Address", "OSCアドレス"),
  ...
  for (int i = 0; i < deviceCount; i++) {
    ...
    if (devices[i].type == CHAIN_KEY_TYPE_CODE) {
      ...
      if (encoderMigrationSave &&
          (!server.hasArg("ev2c_" + idx) ||
           server.arg("ev2c_" + idx) != "1")) {
        sendUiResult(
            400, tr("Save error", "保存エラー"),
